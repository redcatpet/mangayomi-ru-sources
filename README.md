...
## Реальный статус (v0.8.4, проверено через E2E с фактической загрузкой контента)
...
Senkuro-семейство использует **OAuth2-PKCE** с центральным auth-сервером `senkuro.org/oauth/authorize` — там нет публичного login/password endpoint, и login-flow требует браузер-редирект который Mangayomi-расширение не может выполнить.

Поэтому единственный способ передать сессию из браузера в приложение — это **cookie** (поле `Session cookie` в настройках источника):

1. Войди на сайт (senkuro.com / senkognito.com / shiruho.com) — куки выдаются автоматически после OAuth-входа
2. F12 → Application → Cookies → выбрать домен
3. Скопируй ВСЕ cookies одной строкой `key1=value1; key2=value2; ...`
4. Вставь в настройки источника

Без cookie работает публичный контент. Cookie нужна для:
- 18+ манги, скрытой от анонимов
- DMCA-блочного контента в твоём регионе
- Платных глав / эксклюзивов

> Это ограничение Mangayomi — приложение не предоставляет API для встраивания login-формы, OAuth-редиректа или WebView-сессии в JS-расширения. Других вариантов нет.
