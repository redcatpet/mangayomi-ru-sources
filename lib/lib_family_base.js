...
async function libMangaDetail(client, source, siteId, slug) {
    const infoUrl = `${source.apiUrl}/manga/${slug}?fields[]=chap_count&fields[]=summary&fields[]=genres&fields[]=authors&fields[]=artists`;
    ...
    if (infoRes.statusCode !== 200) {
        ...
        return { name: slug, imageUrl: "", description: `(Ошибка загрузки: ${detail})`, status: 5, genre: [], chapters: [] };
    }
    const info = JSON.parse(infoRes.body).data;
    const chRes = await libGetWithFallback(client, `${source.apiUrl}/manga/${slug}/chapters`, source, siteId);
...
async function libMangaPageList(client, source, siteId, url) {
    ...
    // Pick image server. Pages url like "//manga/foo/01.jpg" — must prepend host without
    // creating a double slash.
    let prefix = "https://img2.imglib.info"; // safe default for site_id=1/3
    if (siteId === 4) prefix = "https://img2h.hentaicdn.org"; // HentaiLib
    if (siteId === 2) prefix = "https://img2.hentaicdn.org"; // SlashLib
    try {
        ...
        const filtered = servers.filter(s => (s.site_ids || []).includes(siteId));
        const chosen = filtered.find(s => s.id === prefId) || filtered.find(s => s.id === "main") || filtered[0];
        if (chosen && chosen.url) prefix = chosen.url;
    } catch (e) { /* keep default */ }

    // Strip trailing slash on prefix; page URL starts with `//` (path-style) — convert to `/`.
